...
------------- | ------------- | -------------
[**ExportConnectScript**](ExportsAPI.md#ExportConnectScript) | **Get** /exports/{export}/connect-script | Retruns commands to connect to export
...
> []string ExportConnectScript(ctx, export).Os(os).Authorization(authorization).Execute()

Retruns commands to connect to export
...
func main() {
	...
	resp, r, err := apiClient.ExportsAPI.ExportConnectScript(context.Background(), export).Os(os).Authorization(authorization).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExportsAPI.ExportConnectScript``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ExportConnectScript`: []string
	fmt.Fprintf(os.Stdout, "Response from `ExportsAPI.ExportConnectScript`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiExportConnectScriptRequest struct via the builder pattern
